...
#=============================================================
# 原始文件：https://raw.githubusercontent.com/P3TERX/SSH-Key-Installer/master/key.sh
# Description: Install SSH keys via GitHub, URL or local files
# Author: P3TERX
# 来源Blog: https://p3terx.com/archives/ssh-key-installer.html
```
用法说明：
...
事先在我的alt369的github账户上传公钥，并保存相应的私钥在google drive，位置：D:\googleDrive\sync\Linux VPS\id_ecdsa_521，供ssh客户端使用。多个vps使用同一个密钥对。
使用algt369的github公钥，覆盖原有的vps服务器上的公钥，并将ssh的默认端口改成222端口，同时禁用账号密码登录。
我实际使用的代码：bash <(curl -fsSL git.io/key.sh) -og alt369 -p 2222 -d
