#!/bin/sh
# 来自https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh，20241225
...
# Other options
# CHSH: Change Shell - 如果设置为 "yes"，安装脚本会自动将默认 shell 更改为 zsh
# 可以通过设置 CHSH=no 来禁用此行为，这样可以保持当前的默认 shell 不变
CHSH=${CHSH:-yes}

# RUNZSH: Run Zsh - 如果设置为 "yes"，安装完成后会立即启动 zsh
# 可以通过设置 RUNZSH=no 来禁用此行为，这样安装后会继续使用当前的 shell 会话
RUNZSH=${RUNZSH:-yes}

# KEEP_ZSHRC: Keep existing .zshrc - 如果设置为 "yes"，将保留现有的 .zshrc 文件
# 默认为 "no"，会用 oh-my-zsh 的模板替换现有的 .zshrc 文件
